  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 overflow-y-auto">
      <motion.div 
        ...
        className="bg-white rounded-3xl shadow-xl border border-slate-100 max-w-3xl w-full max-h-[90vh] overflow-y-auto"
      >
        {/* Header */}
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50 rounded-t-3xl sticky top-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Preencha as informações para o catálogo da imobiliária</p>
            </div>
          </div>
          <button 
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Codigo */}
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-2">Código do Imóvel</label>
              <input 
                type="text"
                name="code"
            ...
            {/* Tipo */}
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-2">Tipo de Imóvel</label>
              <select
                name="type"
            ...
            {/* Status */}
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-2">Status Atual</label>
              <select
                name="status"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Preco */}
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-2">Valor de Negociação (R$)</label>
              <input 
                type="number"
                name="price"
                value={formData.price || 0}
                onChange={handleChange}
                className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-hidden focus:ring-1 focus:ring-indigo-500 font-bold text-slate-900"
              />
            </div>

            {/* Area Construida */}
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-2">Área Construída (m²)</label>
              <input 
                type="number"
                name="builtArea"
            ...
            {/* Area Terreno */}
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-2">Área do Terreno (m²)</label>
              <input 
                type="number"
                name="landArea"
          ...
          <div className="grid grid-cols-3 gap-4 p-4 bg-slate-50 rounded-2xl">
            <div>
              <label className="block text-[11px] font-black text-slate-400 uppercase tracking-wider mb-1.5 text-center">Quartos / Dorms</label>
              <input 
                type="number"
                name="bedrooms"
                min="0"
                ...
                onChange={handleChange}
                className="w-full text-center px-4 py-2 rounded-xl bg-white border border-slate-200 text-sm focus:outline-hidden focus:ring-1 focus:ring-indigo-500 font-bold"
              />
            </div>

            <div>
              <label className="block text-[11px] font-black text-slate-400 uppercase tracking-wider mb-1.5 text-center">Banheiros</label>
              <input 
                type="number"
                name="bathrooms"
                min="0"
            ...
            <div>
              <label className="block text-[11px] font-black text-slate-400 uppercase tracking-wider mb-1.5 text-center">Vagas de Garagem</label>
              <input 
                type="number"
                name="garage"
                min="0"
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1">Endereço Literal</label>
                <input 
                  type="text"
                  name="address"
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-2">Descrição Executiva do Imóvel</label>
              <textarea
                name="description"
                rows={3}
            ...
            <div>
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider mb-1">Vídeo de Apresentação (Ex: YouTube ou Vimeo URL)</label>
              <input 
                type="text"
                name="videoUrl"
          ...
          {/* Photo Management */}
          <div className="space-y-3">
            <label className="block text-xs font-black text-slate-500 uppercase tracking-wider">Mídia e Fotos do Imóvel</label>
            
            {/* Predefined photo library options to make cataloging quick */}
            <div className="p-3 bg-indigo-50/50 rounded-2xl">
              <span className="text-[11px] text-indigo-700 font-bold block mb-2">Selecione fotos de demonstração rápida:</span>
                ...
                {PRESET_IMAGES.map((img) => {
                  ...
                  return (
                    <button
                      ...
                      className={`text-xs px-2.5 py-1.5 rounded-lg border transition-all flex items-center gap-1.5 ${isSelected ? 'bg-indigo-600 border-indigo-600 text-white font-bold' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'}`}
                    >
            ...
            <div className="flex gap-2">
              <input 
                ...
                className="flex-1 px-4 py-2 rounded-xl border border-slate-200 text-xs focus:outline-hidden"
              />
              <button
                type="button"
            ...
            <div 
              ...
              onDrop={handleDrop}
              className="border-2 border-dashed border-slate-200 rounded-2xl p-6 text-center hover:bg-slate-50 cursor-pointer transition"
            >
              <div className="flex flex-col items-center gap-1.5">
                <ImageIcon className="w-8 h-8 text-slate-400" />
                <p className="text-xs text-slate-700 font-medium">Arraste e solte arquivos aqui para simular upload</p>
                <p className="text-[10px] text-slate-400">Suporta JPG, PNG</p>
              </div>
            </div>
            ...
            {formData.images && formData.images.length > 0 && (
              <div className="grid grid-cols-5 gap-3 mt-4">
                {formData.images.map((imgUrl, index) => (
                  <div key={index} className="relative group rounded-xl overflow-hidden border border-slate-100 aspect-video bg-slate-100">
                    <img src={imgUrl} alt="" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    <button
                      type="button"
                      onClick={() => handleRemoveImage(index)}
                      className="absolute top-1 right-1 p-1 bg-red-600 hover:bg-red-700 text-white rounded-md opacity-0 group-hover:opacity-100 transition duration-200"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                    <span className="absolute bottom-1 left-1 bg-slate-950/70 text-[9px] px-1.5 rounded-sm text-white">
                      Foto {index + 1}
                    </span>
                  </div>
                ))}
          ...
          {/* Action Row */}
          <div className="pt-4 border-t border-slate-100 flex gap-3 justify-end sticky bottom-0 bg-white z-10 py-2">
            <button
              type="button"
            ...
            <button
              ...
              className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-sm shadow-xs transition"
            >
              Salvar Registro
            </button>
          </div>
